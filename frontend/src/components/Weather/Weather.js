...
export default function Weather() {
  ...
  useEffect(() => {
    async function getWheather() {
      ...
      setStateWeather({ temperature, clouds, wind, iconWeather  } );
    }
    getWheather();
  }, [])
  ...
  return (
    <div>
      <p> Температура за бортом: { weather.temperature } С, </p>
      <p> {weather.clouds},</p>
      <p> cила ветра: {weather.wind} м/с</p>
      <div> <img style={{width: "20%"}} src={`https://openweathermap.org/img/wn/${weather.iconWeather}@2x.png`} alt="icon" /></div>
    </div>
  )
}
